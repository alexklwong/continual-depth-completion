    ...
    def compute_loss(self,
        ...
        for s, (output_depth, output_uncertainty) in enumerate(zip(output_depths, output_uncertainties)):
            ...
            if 'uncertainty' in self.decoder_type and output_uncertainty is not None:
                ...
                if 'supervised_l1' in loss_func or 'supervised_l1_normalized' in loss_func:
                    loss_supervised += w * losses.l1_with_uncertainty_loss_func(
                        src=output_depth,
                        tgt=target_depth,
                        uncertainty=output_uncertainty,
                        w=validity_map_target_depth)
                elif 'supervised_l2' in loss_func:
                    loss_supervised += w * losses.l2_with_uncertainty_loss_func(
                        src=output_depth,
                        tgt=target_depth,
                        uncertainty=output_uncertainty,
                        w=validity_map_target_depth)
                ...
                if 'supervised_l1_normalized' in loss_func:
                    ...
                    loss_supervised += w * losses.l1_loss_func(
                        ...
                        tgt=target_depth,
                        w=validity_map_target_depth)
                elif 'supervised_l2_normalized' in loss_func:
                    loss_supervised += w * losses.l2_loss_func(
                        src=output_depth,
                        tgt=target_depth,
                        w=validity_map_target_depth,
                        normalize=True)
