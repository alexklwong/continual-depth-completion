...
def l1_loss_func(src, tgt, w, normalize=False):
    ...
    Arg(s):
        ...
        w : torch.Tensor[float32]
            weights for penalty
        normalize : bool
            if set, then normalized loss w.r.t. to target
    Returns:
...
def l2_loss_func(src, tgt, w, normalize=False):
    '''
...
def l2_with_uncertainty_loss_func(src, tgt, uncertainty, w):
    '''
    Computes the l2 loss with uncertainty between source and target

    1/2 (((y - mu) / sigma ** 2) + log sigma ** 2)

    1/2 (((y - mu) ** 2 / exp(sigma*) ** 2) + 2 sigma*)
    ...
    Arg(s):
        ...
        w : torch.Tensor[float32]
            weights for penalty
    Returns:
        float : mean L2 penalty with uncertainty
    '''

    loss = w * (1 / 2.0) * ((tgt - src) ** 2 / (torch.exp(uncertainty) ** 2) + 2 * uncertainty)
