...
def run(image_path,
    ...
    assert os.path.exists(restore_paths_model), \
        'Model restore path does not exist: {}'.format(restore_paths_model)
    ...
    output_path = os.path.join(output_dirpath, 'outputs')
    ...
    if intrinsics_path is not None:
        ...
        intrinsics_paths = [None] * n_sample

    # Make sure number of paths match across inputs
    for paths in [sparse_depth_paths, ground_truth_paths, intrinsics_paths]:
    ...
    model.restore_model(restore_paths_model)
    model.eval()
    ...
    log_system_settings(
        ...
        restore_paths_depth_model=restore_paths_model,
        # Hardware settings
        device=device,
        n_thread=1)
    ...
    for idx, inputs in tqdm.tqdm(enumerate(dataloader), desc='Evaluating', total=n_sample):
        ...
        # Save to output
        if save_outputs:
            ground_truth_image = np.squeeze(ground_truth.cpu().numpy())
            ...
            if is_available_ground_truth:
                ground_truth_path = os.path.join(ground_truth_dirpath, depth_filename)
                data_utils.save_depth(ground_truth_image, ground_truth_path)

            output_depth_path = os.path.join(output_depth_dirpath, depth_filename)
            data_utils.save_depth(output_depth, output_depth_path)

            sparse_depth_path = os.path.join(sparse_depth_dirpath, depth_filename)
    ...
    # Log run time
    log('Completed processing {} samples'.format(n_sample))
...
def train(rank,
    ...
    init_method = 'tcp://localhost:{}'.format(os.environ['MASTER_PORT'])
    ...
    model_checkpoint_path = os.path.join(checkpoint_path, '{}'.format(model_name[0]) + '-{}.pth')
    ...
    if rank == 0:
        if not os.path.exists(event_path):
            os.mkdir(event_path)
            os.mkdir(os.path.join(event_path, 'events-train'))
            os.mkdir(os.path.join(event_path, 'events-test'))
    ...
    train_transforms_occlusion = Transforms(
    ...
    if rank == 0:
        ...
        log_training_settings(
            ...
            # Geometric data augmentations
            augmentation_padding_mode=padding_modes,
    ...
    model.distributed_data_parallel(rank)
    model.train()

    iters_grad_acc = 1
    for mn in model_name:
        if 'grad_acc' in mn and int(mn.split('grad_acc')[-1]):
            iters_grad_acc = int(mn.split('grad_acc')[-1])
    ...
    for epoch in range(1, n_epoch + 1):
        ...
        train_sampler = get_sampler(train_dataset, ngpus_per_node=ngpus_per_node, rank=rank, seed=1234)
        ...
        for inputs in train_dataloader:
            ...
            # Lr warmup if applicable
            if lr_warmup_iter != -1 and train_step <= lr_warmup_iter:
                learning_rate = learning_rates[0]
                # Update optimizer learning rates
                for g in optimizer.param_groups:
                    g['lr'] = learning_rate * train_step / lr_warmup_iter
            ...
            [image] = train_transforms_photometric.transform(
                images_arr=[image],
                random_transform_probability=augmentation_probability)

            # Perform occlusion augmentation i.e. point removal, etc.
            [sparse_depth] = train_transforms_occlusion.transform(
                images_arr=[sparse_depth],
                random_transform_probability=augmentation_probability)
            ...
            loss = loss / iters_grad_acc

            loss.backward()
            if (train_step + 1) % iters_grad_acc == 0:
                optimizer.step()
                optimizer.zero_grad()

            # For visualization the ground truth could be clamped

            ground_truth = torch.clamp(ground_truth, 0, max_predict_depth)

            # Log training summary on Tensorboard
            if (train_step % n_step_per_summary) == 0 and rank == 0:
                model.log_summary(
                    summary_writer=train_summary_writer,
                    tag='train-{}'.format(dt_string),
                    step=train_step,
                    image=image,
                    sparse_depth=sparse_depth,
                    output_depth=output_depth[0].detach().clone(),
            ...
            if rank == 0:
                ...
                for k in loss_info.keys():
                    desc += ', {}: {:.3f}'.format(k, loss_info[k])
                pbar.set_description('tb path: {} / loss {} / train {}/{}'.format(event_path, desc, epoch, learning_schedule[-1]))
                pbar.update(1)
...
def log_system_settings(log_path,
                        ...
                        restore_paths_depth_model=None,
    ...
    if restore_paths_depth_model is not None and restore_paths_depth_model != '':
        log('restore_paths_depth_model={}'.format(restore_paths_depth_model),
            log_path)
