...
from uformer import Uformer_3D as UformerBase3DModel
from uformer_kb import KBformer as KBformerBaseModel
from uformer_kb import KBformer_SUM as KBformerBaseModel_SUM
from uformer_kb import KBformer_SUM_3D_half as KBformer3dBaseModel_half
from uformer_kb import KBformer_SUM_3D as KBformer3dBaseModel
from uformer_kb_Nov5 import KBformer_SUM_3D_lastlayer as KBformer_SUM_3D_lastlayer
from uformer_kb import KBformer_SUM_deckb as KBformerBaseModel_SUM_deckb
from uformer_kb_test import KBformer_SUM_test
from uformer_kb_learnable import KBformer_SUM_aggregation
...
class Uformer_Model(object):
    ...
    def __init__(self, min_predict_depth=1.0,
        ...
        if test:
            print('TESTING!!: KBformer with doubled dimension and replace later block with conv block')
            self.model = KBformer_SUM_test(img_size=256,
                                                dd_in=3,
                                                embed_dim=48,
                                                embed_dim_z=16,
                                                win_size=8,
        ...
        if dec_3d:
            if not kb:
                print('UFormer with Orthogonal projection')
                self.model = UformerBase3DModel(img_size=256,
                                                    dd_in=3,
                                                    embed_dim=32,
                                                    win_size=8,
                                                    min_predict_depth=1.0,
                ...
                if last_layer:
                    print('KBFormer with Orthogonal projection in 1/2 resolution')
                    self.model = self.model = KBformer_SUM_3D_lastlayer(img_size=256,
                                                        dd_in=3,
                                                        embed_dim=32,
                                                        win_size=8,
                                                        token_projection='linear',
                    ...
                    print('KBFormer with Orthogonal projection in 1/2 resolution')
                    self.model = KBformer3dBaseModel_half(img_size=256,
                                                        dd_in=3,
                                                        embed_dim=32,
                                                        win_size=8,
                                                        token_projection='linear',
                    ...
                    print('KBFormer with Orthogonal projection')
                    self.model = KBformer3dBaseModel(img_size=256,
                                                        dd_in=3,
                                                        embed_dim=32,
                                                        win_size=8,
                                                        token_projection='linear',
            ...
            if aggregation:
                print('KBFormer with aggregation')
                self.model = KBformer_SUM_aggregation(img_size=256,
                                                        dd_in=3,
                                                        embed_dim=32,
                                                        win_size=8,
                                                        token_projection='linear',
                ...
                print('KBFormer with encoder decoder spatial encoding')
                self.model = KBformerBaseModel_SUM_deckb(img_size=256,
                                                    dd_in=3,
                                                    embed_dim=32,
                                                    win_size=8,
                                                    token_projection='linear',
                ...
                print('KBFormer with encoder spatial encoding')
                self.model = KBformerBaseModel_SUM(img_size=256,
                                                    dd_in=3,
                                                    embed_dim=32,
                                                    win_size=8,
                                                    token_projection='linear',
            ...
            print('UFormer')
            self.model = UformerBaseModel(img_size=256,
                                                dd_in=3,
                                                embed_dim=32,
                                                win_size=8,
                                                token_projection='linear',
    ...
    #     w_l1 = w_losses['w_l1'] if 'w_l1' in w_losses else 1.0
    #     w_l2 = w_losses['w_l2'] if 'w_l2' in w_losses else 1.0
    #     w_norm_l1 = w_losses['w_norm_l1'] if 'w_norm_l1' in w_losses else 0.0
    #     w_norm_l2 = w_losses['w_norm_l2'] if 'w_norm_l2' in w_losses else 0.0
    #     w_smoothness = w_losses['w_smoothness'] if 'w_smoothness' in w_losses else 0.0
    ...
    #     norm_l1_loss = w_norm_l1 * loss_utils.norm_l1_loss(
    #         src=output_depth,
    #         tgt=ground_truth,
    #         w=validity_map
    #     )
    ...
    #     norm_l2_loss = w_norm_l2 * loss_utils.norm_l2_loss(
    #         src=output_depth,
    #         tgt=ground_truth,
    #         w=validity_map
    #     )
    #     loss = l1_loss + l2_loss + norm_l1_loss + norm_l2_loss
    ...
    #         loss_info['l2_loss'] = l2_loss
    #     if w_norm_l1 > 0:
    #         loss_info['norm_l1_loss'] = norm_l1_loss
    #     if w_norm_l2 > 0:
    #         loss_info['norm_l2_loss'] = norm_l2_loss

    #     if w_smoothness > 0 and image is not None:
    #         mask_smoothness = 1-validity_map
    #         loss_smoothness = w_smoothness * loss_utils.smoothness_loss_func(output_depth, image, mask_smoothness)
    #         loss_info['loss_smoothness'] = loss_smoothness
