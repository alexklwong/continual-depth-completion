...
class ENetModel(object):
    ...
    def compute_loss(self, output_depth, ground_truth, image=None, w_losses={}):
        ...
        w_rgb_depth = w_losses['w_rgb_depth'] if 'w_rgb_depth' in w_losses else 0.05
        w_d_depth = w_losses['w_d_depth'] if 'w_d_depth' in w_losses else 0.05
        ...
        rgb_loss = w_rgb_depth * loss_utils.l2_loss(
            src=self.rgb_depth,
            tgt=ground_truth,
            w=validity_map)

        d_loss = w_d_depth * loss_utils.l2_loss(
            src=self.d_depth,
            tgt=ground_truth,
            w=validity_map)

        l2_loss = (1 - w_rgb_depth - w_d_depth) * loss_utils.l2_loss(
            src=output_depth,
            tgt=ground_truth,
            w=validity_map)

        loss = l2_loss + rgb_loss + d_loss

        # Store loss info
        loss_info['rgb_loss'] = rgb_loss
        loss_info['d_loss'] = d_loss
