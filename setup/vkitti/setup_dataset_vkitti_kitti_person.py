...
KITTI_TRAIN_SPARSE_DEPTH_DIRPATH = os.path.join(KITTI_ROOT_DIRPATH, 'train_val_split', 'sparse_depth', 'train')
...
Output directory
'''
OUTPUT_ROOT_DIRPATH = os.path.join('data', 'virtual_kitti_derived-person')
TRAIN_SUPERVISED_REF_DIRPATH = os.path.join('training', 'virtual_kitti-kitti-person', 'supervised')
TRAIN_UNSUPERVISED_REF_DIRPATH = os.path.join('training', 'virtual_kitti-kitti-person', 'unsupervised')
TEST_REF_DIRPATH = os.path.join('testing', 'virtual_kitti-kitti-person')
...
for n in range(5, 9):
    kitti_sparse_depth_paths += glob.glob(
            os.path.join(KITTI_TRAIN_SPARSE_DEPTH_DIRPATH, '2011_09_28_drive_00{}*'.format(n), KITTI_SPARSE_DEPTH_REFPATH, '*', '*.png'))
